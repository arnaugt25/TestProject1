...
class Songs
{
    ...
    //  id_song | song_name | artist  | song_path
    ...
    {
        $query = "SELECT id_song, song_name, artist, song_path FROM songs";
        $stm = $this->sql->prepare($query);
        $stm->execute();
    ...
    {
        $query = "SELECT song_name, artist, song_path FROM songs WHERE id_song = :id_song;";
        $stm = $this->sql->prepare($query);
    ...
    public function delete($id) {
        try {
            $stmt = $this->sql->prepare("DELETE FROM songs WHERE id_song = :id_song");
            $stmt->execute([':id_song' => $id]);
            return true;
